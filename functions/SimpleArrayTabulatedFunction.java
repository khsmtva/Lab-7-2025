...
public class SimpleArrayTabulatedFunction implements TabulatedFunction, java.io.Serializable {
    private FunctionPoint[] points;
    private int pointsCount;
    
    // Конструктор с массивом точек
    public SimpleArrayTabulatedFunction(FunctionPoint[] points) {
        if (points.length < 2) {
    ...
    public SimpleArrayTabulatedFunction(double leftX, double rightX, int pointsCount) {
        if (leftX >= rightX) {
    ...
    public SimpleArrayTabulatedFunction(double leftX, double rightX, double[] values) {
        if (leftX >= rightX) {
            throw new IllegalArgumentException("Левая граница должна быть меньше правой");
        }
        if (values.length < 2) {
    ...
    // === ТОЛЬКО БАЗОВЫЕ МЕТОДЫ Object (БЕЗ Externalizable!) ===
    ...
    public boolean equals(Object o) {
        ...
        // Оптимизация для SimpleArrayTabulatedFunction
        if (o instanceof SimpleArrayTabulatedFunction) {
            SimpleArrayTabulatedFunction other = (SimpleArrayTabulatedFunction) o;
            for (int i = 0; i < pointsCount; i++) {
    ...
    public Object clone() {
        try {
            SimpleArrayTabulatedFunction clone = (SimpleArrayTabulatedFunction) super.clone();
            ...
            return new SimpleArrayTabulatedFunction(clonedPoints);
        }
    }
    @Override
    ...
    public static class SimpleArrayTabulatedFunctionFactory implements TabulatedFunctionFactory {
        @Override
        public TabulatedFunction createTabulatedFunction(double leftX, double rightX, int pointsCount) {
            return new SimpleArrayTabulatedFunction(leftX, rightX, pointsCount);
        }
        
        @Override
        public TabulatedFunction createTabulatedFunction(double leftX, double rightX, double[] values) {
            return new SimpleArrayTabulatedFunction(leftX, rightX, values);
        }
        
        @Override
        public TabulatedFunction createTabulatedFunction(FunctionPoint[] points) {
            return new SimpleArrayTabulatedFunction(points);
        }
    }
}
